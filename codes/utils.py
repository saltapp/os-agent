...
def load_config(config_path="codes/config.json"):
    """
    加载配置文件，如果不存在则使用模板创建默认配置文件
    ...
    try:
        ...
        if os.path.exists(template_path):
            ...
            print(f"已创建默认配置文件: {config_path}，请修改其中的敏感信息")
            return config
        else:
            raise FileNotFoundError(f"未找到配置文件和模板: {config_path}")